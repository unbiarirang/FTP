		...
		if (user_table[pid].pid == 0) {
			while (true) {
                ...
                if (strncmp("USER", sentence, 4) == 0)
                    ...
                    RETR(sentence, &status, g_connfd, listenfd, connfd, &conn_info, &transfer_files, &transfer_bytes, &startByte);
				else if (strncmp("STOR", sentence, 4) == 0)
                    STOR(sentence, &status, g_connfd, listenfd, connfd, &conn_info, &transfer_files, &startByte);
				...
				else if (strncmp("REST",sentence, 4) == 0)
                    REST(sentence, &status, &startByte);
				else
